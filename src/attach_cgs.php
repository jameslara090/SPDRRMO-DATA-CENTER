...
<?php
    ...
    $stmt = $conn->prepare("SELECT * FROM cgs_attachfile WHERE typhoon_name='".$tname."'  order by addeddate DESC");
    $stmt->execute(); 
    $resulttf = $stmt->fetchAll();

    try{
        $stmt = $conn->prepare("UPDATE cgs_attachfile SET notif=1 WHERE typhoon_name='".$tname."'  ");
        $stmt->execute();
    }
                    ...
                    <div class="table-responsive">
                            ...
                            <table id="data-table" class="table">
                                ...
                                <?php
                                    ...
                                    $row = $conn->query("SELECT * FROM `cgs_attachfile` WHERE typhoon_name='".$tname."' and mun='".$mun."' ORDER BY addeddate DESC") or die(mysqli_error());
                                ...
                                ?>
                                    ...
                                    ?>
                                        <td style="padding:0px;width:100px;"><a title="Download File" href="upload_file/download_cgs.php?file=<?php echo $name[2]?>" class="btn btn-primary"><i class="zmdi zmdi-download"></i> Download</a></td>
                                        <td style="border:1px solid #2b4c4a; padding:2px; vertical-align : middle;text-align:left; color:#a8b6b5; padding-left:8px;"><?php echo $fetch['filename']?></td>
